...
{	
	...
	ShotGroup sg;   //子弹组
	WallGroup wg;
	String DefaultFile="Save.txt";  //保存读取的文件名
	...
	public int PutRecord(){		
		return PutRecord(DefaultFile);
	}
	public int PutRecord(String Filename){
		try {
	...
	public int GetRecord(){
		return GetRecord(DefaultFile);
	}
	public int GetRecord(String Filename){
		try {
...
class Bomb extends MyObject //爆炸效果类
{
	...
	public Bomb(int x,int y)//构造函数
	{
		super(x,y);
		try {
			image1=ImageIO.read(new File("bomb_1.gif"));//三个爆炸效果图片
