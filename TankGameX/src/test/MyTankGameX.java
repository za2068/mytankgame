...
public class MyTankGameX extends JFrame implements ActionListener{
	...
	String GameFile="map.txt"; //默认开始游戏读取文件
	...
	public void StartGame(){
		...
		startmp.setThreadLock(false); //游戏界面，锁上开始界面和结束界面线程
		exitmp.setThreadLock(false);
		if(FirstStart)	{
			mp.myRecord.PutRecord("map.txt");//第一次启动写入map。txt
			Thread p = new PlayWave("start.wav");
			p.start();
		}
		else ReStartGame(GameFile);			
		FirstStart=false; //第一次启动标志位，置否
		try {
			this.remove(startmp);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void ReStartGame(String GameFile){
		...
		mp.myRecord.GetRecord(GameFile);
		for(int i=0;i<mp.enemyGroup.Container.size();i++){
	...
	public void actionPerformed(ActionEvent a) {
		...
		if(a.getActionCommand() == "Save"){
			if(mp.myRecord.PutRecord() !=0) System.out.println("operation save failed!");
		}
	...
	public void run(){	
		while(true){
			...
			if(this.isThreadLock()){		
				...
				shotGroup.HitShot(shotGroup, bombGroup);
